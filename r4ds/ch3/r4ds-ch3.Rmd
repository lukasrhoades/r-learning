...
Using distance and airtime (suggested solution)
```{r}
flights |> 
...
Apparently, no, since filter based on condiiton not row number
My original answer:
...
flights |>  # Name all the columns
  select(dep_time, dep_delay, arr_time, arr_delay) 

flights |>  # Creative (from solution guide)
...
Arranges data in ascending (for min) or descending (for max) order, but doesn't slice data with the value of given variable
```{r}
flights |>
  group_by(dest) |> 
  relocate(dest) |> 
